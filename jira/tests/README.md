...
target-permission refusal, partial mutation, recalculation timeout and POST resume, read-only
status, verified success, exact rollback, and manual recovery. The HTTP suite verifies every
Doctor declaration, admin restriction, explicit authentication check, JSON media type and
size boundary, query-key boundary, and absence of a mutating GET. The mutation-probe suite
uses a synthetic adapter and proves that refusal happens before a write and restoration runs
in `finally`. None of these suites enables or invokes a live writer.

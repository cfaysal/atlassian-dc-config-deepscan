...
is `MANUAL_RECOVERY_REQUIRED`. If the writer explicitly proves that it applied nothing, the
operation becomes terminal `MUTATION_FAILED`. A missing receipt or writer exception has an
unknown target effect and therefore becomes `MANUAL_RECOVERY_REQUIRED`. GET Status reads the
journal without changing it. Repeating the identical POST Apply for a `PENDING` operation
resumes verification under the locks; terminal replays are read-only, while reusing an
operation ID with different inputs is rejected.
